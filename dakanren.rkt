...
(define (da-constrain-absents st var=>cxs as)
  (forf st = st var=>cxs = var=>cxs
        (list ground tm) <- as
        #:break (not var=>cxs)
        (letn loop (values st var=>cxs tm) = (values st var=>cxs tm)
          (values st tm) = (muk-walk st tm)
          (match tm
            ((? muk-var?)
             (values st (hash-update
                          var=>cxs tm
                          (lambda (cxs) (constraints-absents-add cxs ground))
                          constraints-empty-v)))
            ((cons h0 t0)
             (lets (values st var=>cxs) = (loop st var=>cxs h0)
                   (if var=>cxs
                     (loop st var=>cxs t0)
                     (values st var=>cxs))))
            (_ (values st (if (eqv? ground tm) #f var=>cxs)))))))
...
(define (da-constrain-types st var=>cxs ts)
  (forf st = st var=>cxs = var=>cxs
        (list tag tm) <- ts
        #:break (not var=>cxs)
        (values st tm) = (muk-walk st tm)
        (values st
          (if (muk-var? tm)
            (lets
              cxs = (hash-ref var=>cxs tm constraints-empty-v)
              tag0 = (constraints-type cxs)
              (if tag0
                (and (eq? tag0 tag) var=>cxs)
                (hash-set var=>cxs tm (constraints-type-set cxs tag))))
            (and (or (and (symbol? tm) (eq? tag 'sym))
                     (and (number? tm) (eq? tag 'num))) var=>cxs)))))
(define (typeo tag tm)
